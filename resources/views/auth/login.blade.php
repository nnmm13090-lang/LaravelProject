...
@section('title', 'Sign In - ' . config('app.name', 'The Desk'))
    ...
    <div class="relative mx-auto flex h-screen max-w-7xl items-center px-6 py-6 lg:px-10 lg:py-8">
        <div class="grid w-full items-center gap-8 lg:grid-cols-[0.95fr_1.05fr]">
            <div class="max-w-xl">
                ...
                <p class="mt-12 text-[0.72rem] font-bold uppercase tracking-[0.34em] text-accent2">Welcome Back</p>
                <h1 class="mt-5 font-display text-5xl font-bold leading-[0.96] tracking-tight text-ink lg:text-7xl">
                    Sign in to your corner of the internet.
                </h1>
            </div>
            ...
            <div class="relative">
                ...
                <div class="relative rounded-[2rem] border border-white/70 bg-white/78 p-7 shadow-[0_30px_80px_rgba(89,64,42,0.14)] backdrop-blur-md lg:p-9">
                    <div class="mb-8 flex items-start justify-between gap-4">
                        <div>
                            <p class="text-[0.68rem] font-bold uppercase tracking-[0.28em] text-accent">Sign In</p>
                            <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-ink">Welcome back</h2>
                        </div>
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-5">
                        ...
                        <div>
                            <div class="mb-2 flex items-center justify-between gap-4">
                                <label for="password" class="text-[0.62rem] font-bold uppercase tracking-[0.2em] text-muted">Password</label>
                                @if(Route::has('password.request'))
                                    <a href="{{ route('password.request') }}" class="text-[0.72rem] font-semibold text-accent2 hover:text-accent">
                                        Forgot password
                                    </a>
                                @endif
                            </div>
                        ...
                        <label for="remember" class="flex items-center gap-3 rounded-2xl bg-[#f6ede1] px-4 py-3 text-sm text-charcoal">
                    ...
                    <div class="mt-8 rounded-[1.5rem] bg-[#f4e8da] px-5 py-4">
                        <p class="text-sm text-charcoal">
                            New here?
                            <a href="{{ route('register') }}" class="font-semibold text-accent2 hover:text-accent">Create an account</a>
                        </p>
                    </div>
                </div>
            </div>
